...
> 分组约定：**高质量**（`51` 端口段 = ISP IP：住宅/原生/家宽）、**普通**（`52` 端口段 = 机房/转发）、**代理池**（独立第三组，按节点分组名「代理池」统计）。
...
- **批量导入**：订阅一键导入，自动按名称判定高质量（51）/普通（52）端口段，去重 + 分组继承
- **三组分组**：高质量（`51` 段 ISP IP）/ 普通（`52` 段）/ 代理池（独立分组名），一键重新分配端口
